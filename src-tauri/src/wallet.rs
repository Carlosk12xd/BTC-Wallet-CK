...
use bitcoin::{consensus::encode::{deserialize, serialize_hex}, Address, Amount, FeeRate, Network, Transaction};
...
#[derive(Debug, Serialize)]
pub struct RawTransactionReview {
    pub txid: String,
    pub network: String,
    ...
    pub ready_for_broadcast_review: bool,
    pub status: String,
    pub warning: String,
}

#[derive(Debug, Deserialize)]
pub struct BackendBroadcastInput {
    pub tx_hex: String,
    pub network: String,
...
#[derive(Debug, Serialize)]
pub struct BackendBroadcastResult {
    pub txid: String,
    pub network: String,
    pub esplora_url: String,
    pub explorer_url: String,
...
impl MiningWallet {
    ...
    pub fn info(&self) -> Result<WalletInfo> {
        ...
        Ok(WalletInfo {
            ...
            warning: "Development wallet. v0.99.5 adds backend Esplora sync and wallet-built signed transaction creation. Test on signet/testnet first before using meaningful mainnet funds.".to_string(),
        })
    }
    ...
    pub fn next_receive_address(&mut self) -> Result<ReceiveAddressInfo> {
        ...
        Ok(ReceiveAddressInfo {
            ...
            index,
            network: network_id(self.network).to_string(),
            warning: "Receive address generated locally. v0.99.5 can sync the wallet with public Esplora APIs from the Send/Receive tab. Save encrypted wallet storage to persist the address index after restart.".to_string(),
        })
    }
    ...
    pub fn create_send_draft(&self, input: SendDraftInput) -> Result<SendDraft> {
        ...
        Ok(SendDraft {
            ...
            warning: "Do not use this to send real BTC yet. v0.99.5 can build and sign a real wallet transaction after backend sync. Review the signed transaction before broadcasting.".to_string(),
        })
    }
...
pub fn review_raw_transaction_hex(tx_hex: &str, network_choice: &str) -> Result<RawTransactionReview> {
    let network = parse_network(network_choice)?;
    let tx = parse_raw_transaction_hex(tx_hex)?;
    let txid = tx.compute_txid().to_string();
    let output_sats = tx.output.iter().map(|output| output.value.to_sat()).sum::<u64>();
    Ok(RawTransactionReview {
        txid: txid.clone(),
        network: network_id(network).to_string(),
        input_count: tx.input.len(),
        output_count: tx.output.len(),
        output_sats,
        tx_hex_len: tx_hex.trim().len(),
        explorer_url: format!("{}/tx/{txid}", explorer_base_url(network)),
        ready_for_broadcast_review: true,
        status: "Raw transaction decoded successfully. Review the transaction details and only broadcast if you intentionally want to spend funds.".to_string(),
        warning: "Raw transaction review cannot prove the transaction belongs to your wallet or that every output is intended. Verify recipient, amount, change, fee, and network before broadcasting.".to_string(),
    })
}

pub fn broadcast_raw_transaction_backend(input: BackendBroadcastInput) -> Result<BackendBroadcastResult> {
    let network = parse_network(&input.network)?;
    let tx = parse_raw_transaction_hex(&input.tx_hex)?;
    let txid = tx.compute_txid().to_string();
    let url = input.esplora_url
        .filter(|value| !value.trim().is_empty())
        .unwrap_or_else(|| default_esplora_url(network).to_string());
    let client = esplora_client::Builder::new(&url).build_blocking();
    client.broadcast(&tx)?;

    Ok(BackendBroadcastResult {
        txid: txid.clone(),
        network: network_id(network).to_string(),
        esplora_url: url,
        explorer_url: format!("{}/tx/{txid}", explorer_base_url(network)),
        status: "Transaction broadcast accepted by the backend Esplora client.".to_string(),
        warning: "Broadcast is final if accepted by the Bitcoin network. Keep the TXID and verify confirmation in a block explorer.".to_string(),
    })
}

fn parse_raw_transaction_hex(tx_hex: &str) -> Result<Transaction> {
    let bytes = decode_hex(tx_hex.trim())?;
...
fn decode_hex(input: &str) -> Result<Vec<u8>> {
    ...
    if clean.is_empty() {
        return Err(anyhow!("Raw transaction hex is empty"));
    }
    if clean.len() % 2 != 0 {
        return Err(anyhow!("Raw transaction hex has an odd number of characters"));
    }
    let mut bytes = Vec::with_capacity(clean.len() / 2);
    let chars: Vec<char> = clean.chars().collect();
    for pair in chars.chunks(2) {
        let high = pair[0].to_digit(16).ok_or_else(|| anyhow!("Invalid hex character: {}", pair[0]))?;
        let low = pair[1].to_digit(16).ok_or_else(|| anyhow!("Invalid hex character: {}", pair[1]))?;
        bytes.push(((high << 4) | low) as u8);
    }
    Ok(bytes)
}

pub fn explorer_base_url(network: Network) -> &'static str {
    match network {
